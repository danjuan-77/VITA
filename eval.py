...
def _get_rawvideo_dec(
    ...
    Returns:
        patch_images: Tensor of shape (T, C, H, W)
        slice_len: Number of frames
    """
    if s is None:
    ...
    if num_frames <= 0:
        raise ValueError(f"Invalid frame range: start={f_start}, end={f_end}")

    stride = max(1, int(round(fps / video_framerate)))
    positions = list(range(f_start, f_end + 1, stride))
    if len(positions) > max_frames:
        positions = np.linspace(0, len(positions)-1, num=max_frames, dtype=int).tolist()
    elif len(positions) < min_frames:
        positions = np.linspace(0, len(positions)-1, num=min_frames, dtype=int).tolist()

    frames = vreader.get_batch(positions).asnumpy()
    ...
    if image_aspect_ratio == "pad":
        ...
        def expand2square(img, bg):
            ...
            return canvas
        pil_imgs = [expand2square(img, mean_color) for img in pil_imgs]

    tensors = [image_processor.preprocess(img, return_tensors="pt")["pixel_values"][0] for img in pil_imgs]
    patch_images = torch.stack(tensors)
    return patch_images, patch_images.size(0)
...
def infer_text_audio(
    ...
    return _generate_response(model, tokenizer, prompt, c, img_zero, audios, "lang")
...
def infer_text_image(
    ...
    img, p_num = dynamic_preprocess(
        img, min_num=1, max_num=12, image_size=448,
    ...
    return _generate_response(model, tokenizer, prompt, c, img_tensor, None, "image")
...
def infer_text_video(
    ...
    return _generate_response(model, tokenizer, prompt, c, vid_tensor, None, "video")
...
def infer_video_with_audio(
    ...
    return _generate_response(model, tokenizer, prompt, c, vid_tensor, audios, "video")
...
def main():
    ...
    # No need to check is_loaded; image_processor is ready to use
